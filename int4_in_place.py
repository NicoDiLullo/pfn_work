...
in the int4 format. We had originally intended to do FP4, but wanted a simulation
without a lookup table, kernels, and B200 or MI350 runs.
Data is loaded (from float64), cast in place to an int4 approximation,
...
scale = np.maximum(amax / FP4_MAX, EPS).astype(np.float32, copy=False)

X_scaled = X/scale
quantized = np.rint(X_scaled)
quant_and_clipped = np.clip(quantized, -FP4_MAX, FP4_MAX)
X = (quant_and_clipped * scale).astype(np.float32, copy=False)
...
#plt.show()
plt.savefig('roc_int4__inplace_final.png')
