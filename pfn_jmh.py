...
def load_and_run(dtype, seed):
    ...
    quarks = Y_test[:,1] == 1
    plt.figure()
    plt.hist(masses[gluons], bins=np.linspace(0, 0.2, 50), density=True, alpha=0.7, label='Gluons')
    plt.hist(masses[quarks], bins=np.linspace(0, 0.2, 50), density=True, alpha=0.7, label='Quarks')
    plt.xlabel('Mass')
    plt.ylabel('Normalized Density')
    plt.title(f'Jet Mass Distribution ({dtype})')
    plt.legend()
    plt.savefig(f'mass_distribution_{dtype}.png')
    plt.close()
    return auc
...
#float16_mult_accs = []
for i in range(1):
    a  = load_and_run(np.float64, seed=42)
...
for i in range(1):
    a = load_and_run(np.float32, seed=42)
...
for i in range(1):
    a = load_and_run(np.float16, seed=42)
